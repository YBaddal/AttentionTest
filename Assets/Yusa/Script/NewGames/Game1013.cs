...
public class Game1013 : MonoBehaviour
{
    Question question;
    public int level;
    public int point;
    ...
    void SetLevel()
    {
        ...
        {
            case 0:
                PrepareImages(1, imageToggle.Count);
                break;
            case 1:
                PrepareTexts(1, textToggle.Count);
                break;
            case 2:
                PrepareImages(1, imageToggle.Count);
                break;
            case 3:
                ...
                break;
            case 4:
                PrepareWords(wordToggle.Count);
                break;
            case 5:
                PrepareImages(2, imageToggle.Count);
                break;
            case 6:
                PrepareImages(1, imageToggle.Count);
                break;
            ...
            case 8:
                ...
                break;
            case 9:
                PrepareImages(Random.RandomRange(2, 4), imageToggle.Count);
                break;
            default:
                PrepareImages(Random.RandomRange(1, 4), imageToggle.Count);
                break;
        }
    }
    void PrepareImages(int questionImgCount, int answerCount)
    {
        while (questionSprites.Count < questionImgCount)
        {
            int random = Random.RandomRange(0, sprites.Count);
            if (!questionSprites.Contains(sprites[random]))
            { 
                questionSprites.Add(sprites[random]);
                questionImg[questionSprites.Count - 1].transform.parent.gameObject.SetActive(true);
                questionImg[questionSprites.Count-1].sprite = sprites[random];
            }
        }

        
        while (answerSprites.Count < answerCount)
        {
            int rnd = Random.RandomRange(0, sprites.Count);
            if (!questionSprites.Contains(sprites[rnd]))
            {
                answerSprites.Add(sprites[rnd]);
                imageToggle[answerSprites.Count - 1].gameObject.SetActive(true);
                imageToggle[answerSprites.Count - 1].transform.GetChild(0).GetComponent<Image>().sprite = sprites[rnd];
            }
        }

        for(int i = 0; i < questionImgCount; i++)
        {
            int correct = Random.RandomRange(0, answerCount);
            imageToggle[correct].transform.GetChild(0).GetComponent<Image>().sprite = questionImg[i].sprite;
        }
    }
    void PrepareTexts(int questionTextCount,int answerCount)
    {
        while (questionStrings.Count < questionTextCount)
        {
            int random = Random.RandomRange(0, strings.Count);
            int randomColor = Random.RandomRange(0, colors.Count);

            if (!questionStrings.Contains(strings[random]))
            {
                questionStrings.Add(strings[random]);
                questionTexts[questionStrings.Count - 1].transform.parent.gameObject.SetActive(true);
                questionTexts[questionStrings.Count - 1].text = strings[random];
                questionTexts[questionStrings.Count - 1].color = colors[randomColor];
            }
        }


        while (answerStrings.Count < answerCount)
        {
            int rnd = Random.RandomRange(0, strings.Count);
            int rndColor = Random.RandomRange(0, colors.Count);

            if (!questionStrings.Contains(strings[rnd]))
            {
                answerStrings.Add(strings[rnd]);
                textToggle[answerStrings.Count - 1].gameObject.SetActive(true);
                textToggle[answerStrings.Count - 1].transform.GetChild(1).GetComponent<Text>().text = strings[rnd];
                textToggle[answerStrings.Count - 1].transform.GetChild(1).GetComponent<Text>().color = colors[rndColor];
            }
        }

        for (int i = 0; i < questionTextCount; i++)
        {
            int correct = Random.RandomRange(0, answerCount);
            textToggle[correct].transform.GetChild(1).GetComponent<Text>().text = questionTexts[i].text;
            textToggle[correct].transform.GetChild(1).GetComponent<Text>().color = questionTexts[i].color;
        }
    }
    void PrepareWords(int answerCount)
    {
        ...
        while (answerStrings.Count < answerCount)
        {
            ...
            if (!answerStrings.Contains(str))
            {
                ...
                wordToggle[answerStrings.Count - 1].gameObject.SetActive(true);
                wordToggle[answerStrings.Count - 1].transform.GetChild(1).GetComponent<Text>().text = str;
            }
        }
